...
Ext.define('Ext.pivot.dimension.Item', {
    ...
    serialize: function() {
        ...
        return Ext.apply(cfg, {
            ...
            renderer: Ext.isFunction(cfg.renderer) ? null : cfg.renderer,
            labelRenderer: Ext.isFunction(cfg.labelRenderer) ? null : cfg.labelRenderer
        });
    },
...
Ext.define('Ext.pivot.feature.PivotStore', {
    ...
    processGrandTotal: function() {
        var me = this,
            ...
            lenD = matrix.leftAxis.dimensions.getCount(),
        ...
        for (i = 0; i < lenT; i++) {
            ...
            if (record instanceof Ext.data.Model) {
                ...
                for (j = 0; j < lenD; j++) {
                    column = matrix.leftAxis.dimensions.getAt(j);
    ...
    processGroupHeaderRecordOutline: function(config) {
        ...
        for (i = 0; i < len; i++) {
            column = matrix.leftAxis.dimensions.getAt(i);
    ...
    processRecordOutline: function(config) {
        var me = this,
            ...
            found = false,
            record = group.record,
            items = me.getMatrix().leftAxis.dimensions,
            len = items.getCount(),
            i, column;
        me.storeInfo[record.internalId] = {
            leftKey: group.key,
...
Ext.define('Ext.pivot.cell.Group', {
    ...
    groupOutlineRenderer: function(config, grid) {
        ...
        if (grid.getMatrix().viewLayoutType == 'compact') {
            ...
            me.innerElement.setStyle(grid.isRTL() ? 'padding-right' : 'padding-left', '0');
        }
        if (config.colspan > 0) {
    ...
    groupCompactRenderer: function(config, grid) {
        var me = this,
            cellCls = '';
        me.innerElement.setStyle(grid.isRTL() ? 'padding-right' : 'padding-left', (me.compactLayoutPadding * config.group.level) + 'px');
...
Ext.define('Ext.pivot.Grid', {
    ...
    onMatrixDataReady: function(matrix) {
        var me = this,
            ...
            leftItems = matrix.leftAxis.items,
            topItems = matrix.topAxis.items,
            i, len, item;
        ...
        if (me.expandedItemsState) {
            len = leftItems.getCount();
            for (i = 0; i < len; i++) {
                item = leftItems.getAt(i);
                if (Ext.Array.indexOf(me.expandedItemsState['rows'], item.key) >= 0) {
                    item.expanded = true;
                    stateApplied = true;
                }
            }
            len = topItems.getCount();
            for (i = 0; i < len; i++) {
                item = topItems.getAt(i);
...
Ext.define('Ext.pivot.plugin.configurator.Container', {
    ...
    initialize: function() {
        ...
        if (me.getFieldType() !== 'all') {
            ...
            me.container.element.on({
                delegate: '.' + Ext.baseCSSPrefix + 'listitem-body',
                tap: 'handleColumnTap',
                scope: me
            });
        }
...
Ext.define('Ext.pivot.plugin.configurator.FormController', {
    ...
    onChangeFilterOperator: function(combo, record) {
        var op = record && record.isModel ? record.get('value') : record,
            ...
            items = view.query('[role=between]'),
            len = items.length,
            i;
        view.down('[role=operator]').setHidden(between);
        for (i = 0; i < len; i++) {
            items[i].setHidden(!between);
        }
    }
});
...
Ext.define('Ext.pivot.plugin.DrillDown', {
    ...
    createPanel: function(store, columns) {
        var me = this;
        return me.getGrid().add(Ext.create('Ext.Sheet', {
            width: me.getWidth(),
            hideOnMaskTap: false,
            enter: 'right',
            exit: 'right',
...
Ext.define('Ext.pivot.plugin.RangeEditor', {
    ...
    onShowView: function(view) {
        this.getGrid().fireEvent('showrangeeditorpanel', view);
    },
    onHideView: function(view) {
