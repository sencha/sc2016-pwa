...
Ext.define('Ext.pivot.Grid', {
    ...
    /**
     * Set a new store to pivot. This works only when using a {@link Ext.pivot.matrix.Local}.
     */
...
Ext.define('Ext.pivot.plugin.configurator.Panel', {
    ...
    getConfigFields: function(dimension) {
        var fields = this.getFields(),
            ...
            i, obj, field;
        for (i = 0; i < len; i++) {
            field = fields.byDataIndex.get(dimension[i].dataIndex);
            if (field) {
                list.push(field);
            }
        }
        ...
        if (pivot.fireEvent('beforemoveconfigfield', this, {
            ...
            if (fromContainer !== toContainer) {
                ...
                if (fromFieldType === 'all' && (toFieldType === 'leftAxis' || toFieldType === 'topAxis')) {
                    ...
                    // We already contain, the field, so don't do anything
                    if (item) {
                        return;
                    }
                    ...
                    // scenario 3
                    if (column.containsFocus) {
                        column.el.suspendFocusEvents();
                        toFocus = column;
                    }
